...
#' @param spe raw count matrix

library(Seurat)
ST_preprogress <- function(spe, n_counts=2, n_feature=200, percent_mito=20){
  ...
  genes_to_keep <- setdiff(names(which(Matrix::rowSums(spe[["Spatial"]]$counts !=0)>n_counts)),mt.genes)
  spe <- subset(spe,
